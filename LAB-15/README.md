...
4. Перейдем к настройке оборудрвания AS2000, для предоставления сервисов конечным пользователям. 
   ####
   Настроим динамическую маршрутизацию между роутерами R3 и R4 по протоколу isis. Роутеры имеют прямой линк, и линки через S9, S10 воспользуемся этим для построения iBGP соседства с резервными маршрутами.
   ```
